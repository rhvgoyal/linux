...
static void show_poll_info(struct seq_file *m, struct io_kiocb *req)
{
	struct io_poll *poll;
	char events_buf[256];
	char file_info[64] = "file=<unknown>";
		...
		if (req->flags & REQ_F_FIXED_FILE) {
			...
			for (unsigned int i = 0; i < req->ctx->file_table.data.nr; i++) {
				if (req->ctx->file_table.data.nodes[i] &&
					...
					snprintf(file_info, sizeof(file_info), "fixed_fd=%u", i);
					break;
				}
			}
		} else {
			/* 
			 * For regular files, show inode and device info for identification.
			 * The original fd number is not readily available since the SQE
			 * has been processed and we only have the file pointer.
			 */
			if (file->f_inode) {
				snprintf(file_info, sizeof(file_info), "ino=%lu", 
					file->f_inode->i_ino);
			}
		}
	}

	seq_printf(m, "  op=%d (POLL_ADD), task_works=%d, events=%s, %s\n",
		   req->opcode,
		   task_work_pending(req->tctx->task),
		   poll_events_to_str(poll->events, events_buf, sizeof(events_buf)),
		   file_info);
}
