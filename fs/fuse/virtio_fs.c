			       ...
			       struct virtio_fs *fs)
{
	...
	fs->vqs = devm_kcalloc(&vdev->dev, fs->nvqs,
				sizeof(fs->vqs[VQ_NOTIFY]), GFP_KERNEL);
	if (!fs->vqs)
		return -ENOMEM;

	vqs = kmalloc_array(fs->nvqs, sizeof(vqs[VQ_NOTIFY]), GFP_KERNEL);
	callbacks = kmalloc_array(fs->nvqs, sizeof(callbacks[VQ_NOTIFY]),
					GFP_KERNEL);
	names = kmalloc_array(fs->nvqs, sizeof(names[VQ_NOTIFY]), GFP_KERNEL);
	if (!vqs || !callbacks || !names) {
		ret = -ENOMEM;
		goto out;
	}

	callbacks[VQ_NOTIFY] = virtio_fs_notifications_done;
	snprintf(fs->vqs[VQ_NOTIFY].name, sizeof(fs->vqs[VQ_NOTIFY].name),
			"notifications");
	INIT_WORK(&fs->vqs[VQ_NOTIFY].done_work,
			virtio_fs_notifications_done_work);
	names[VQ_NOTIFY] = fs->vqs[VQ_NOTIFY].name;
	INIT_LIST_HEAD(&fs->vqs[VQ_NOTIFY].queued_reqs);
	INIT_DELAYED_WORK(&fs->vqs[VQ_NOTIFY].dispatch_work,
...
static int virtio_fs_setup_dax(struct virtio_device *vdev, struct virtio_fs *fs)
{
	...
	/* Get journal region */
	have_journal = virtio_get_shm_region(vdev,
					     &journal_reg,
	...
	have_vertab = virtio_get_shm_region(vdev,
					    &vertab_reg,
	...
	dev_dbg(&vdev->dev, "%s: window kaddr 0x%px phys_addr 0x%llx len %llx\n",
