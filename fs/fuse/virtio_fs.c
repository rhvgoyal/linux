...
static int fsnotify_remote_call(struct inode *inode, uint32_t mask,
								 struct inode *dir, struct qstr *filename)
{
	/*
	 * FSNOTIFY_EVENT_INODE will not work for all the events that can be
	 * generated by fsnotify. For example in fanotify file descriptor mode
	 * we need data of type FSNOTIFY_EVENT_PATH
	 */
	if (mask & (FS_MOVE | FS_CREATE | FS_DELETE)) {
		dir = inode;
		if (mask & FS_MOVED_FROM) {
			return __fsnotify(mask, dir, FSNOTIFY_EVENT_INODE, dir,
					(const struct qstr *)filename, NULL, 1);
		}
		else if (mask & FS_MOVED_TO) {
			return __fsnotify(mask, dir, FSNOTIFY_EVENT_INODE, dir,
					(const struct qstr *)filename, NULL, 2);
		}
		else if (mask & (FS_CREATE | FS_DELETE)) {
			return __fsnotify(mask, dir, FSNOTIFY_EVENT_INODE, dir,
					(const struct qstr *)filename, NULL, 0);
		}
	}

	return __fsnotify(mask, inode, FSNOTIFY_EVENT_INODE, dir,
			  (const struct qstr *)filename, inode, 0);
}
			...
			struct fuse_notify_fsnotify_out *fsnotify_out)
{
	...
	/* Get parent if we have to notify it */
	if (fsnotify_out->notify_parent)
		dir = fuse_ilookup(fc, fsnotify_out->parent, &fm);
	else
		dir = NULL;
	 ...
	 * Do the generation check at this point as well
	 */
	...
	if (fsnotify_out->namelen > 0) {
		...
		filename = (char *)fsnotify_out + sizeof(struct fuse_notify_fsnotify_out);
		filename[name.len] = '\0';
		name.name = filename;
		ret = fsnotify_remote_call(inode, mask, dir, &name);
	} else {
		ret = fsnotify_remote_call(inode, mask, dir, NULL);
	}
